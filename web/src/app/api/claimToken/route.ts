...
const { BACKED_WALLET_ADDRESS, ENGINE_URL, THIRDWEB_SECRET_KEY } = process.env;
...
export async function POST(request: Request) {
  if (!BACKED_WALLET_ADDRESS || !ENGINE_URL || !THIRDWEB_SECRET_KEY) {
  ...
  const resp = await fetch(
    `${ENGINE_URL}/contract/84532/${tokenContractAddress}/erc20/mint-to`,
    {
      ...
      headers: {
        ...
        Authorization: `Bearer ${THIRDWEB_SECRET_KEY}`,
        'x-backend-wallet-address': BACKED_WALLET_ADDRESS,
      },
      body: JSON.stringify({
